...
public class GameService {
    ...
    public GameEntity createNewGame(String playerName) throws IOException {
        ...
        val playerModel = new Player()
                ...
                .withLastMovement(LocalDateTime.now())
    ...
    public GameEntity start(Long gameId)
    {
        ...
        if (model.players.size() == 1 && model.state == GameState.STARTING) {
            model.state = GameState.FINISHED;
        }
        else if (model.state == GameState.STARTING) {
    ...
    @Transactional
    public boolean existGame(Long gameId)
    {
        var gameModel = gameRepository.findById(gameId);
    ...
    public boolean canPlayerPutBomb(Long playerId)
    {
        ...
        if (player.firstBomb == true)
        {
            ...
            if (LocalDateTime.now().isBefore(player.lastBomb.plusNanos(delay * tick * 1000000))) {
                return false;
            }
            return true;
        }
    ...
    public boolean canPlayerMove(Long playerId)
    {
        ...
        else
        {
            ...
            if (LocalDateTime.now().isBefore(player.lastMovement.plusNanos(delay * tick * 1000000)))
            {
                return false;
            }
            return true;
    ...
    public GameEntity putBomb(Long gameId, Long playerId, int posX, int posY) throws IOException{
        var gameModel = gameRepository.findById(gameId);
        var playerModel = playerRepository.findById(playerId);
        if (gameModel == null || playerModel.lives <= 0 || gameModel.state != GameState.RUNNING)
        {
            return null;
        }
